...
@OnlyIn(Dist.CLIENT)
public class RichTextFieldWidget extends Widget implements IRenderable, IGuiEventListener {
	...
	public RichTextFieldWidget(FontRenderer fontIn, int p_i51137_2_, int p_i51137_3_, int p_i51137_4_, int p_i51137_5_,
			ITextComponent msg) {
		this(fontIn, p_i51137_2_, p_i51137_3_, p_i51137_4_, p_i51137_5_, (RichTextFieldWidget) null, msg);
	}

	public RichTextFieldWidget(FontRenderer fontIn, int xIn, int yIn, int widthIn, int heightIn,
			@Nullable RichTextFieldWidget p_i51138_6_, ITextComponent msg) {
	...
	protected IFormattableTextComponent getNarrationMessage() {
		ITextComponent s = this.getMessage();
		return s.getString().isEmpty() ? new StringTextComponent("")
				: new StringTextComponent(I18n.format("gui.narrate.editBox", s, this.text));
	}

	public void setInitialText(String textIn) {
	...
	public void setText(String textIn) {
		if (this.validator.test(textIn)) {
			...
			this.setSelectionPos(this.cursorPosition);
			this.func_212951_d(textIn);
			Networking.INSTANCE.sendToServer(new FindItemsContainingStringRequest(this.getText()));
		}
	}
		...
		if (!this.func_212955_f()) {
			...
			if (Screen.isSelectAll(p_keyPressed_1_)) {
				...
				case 262:
					...
					} else if (!this.field_212956_h && !this.getSelectedText().isEmpty()) {
						int rightSelection = this.cursorPosition > this.selectionEnd ? this.cursorPosition
								: this.selectionEnd;
						this.setCursorPosition(rightSelection);
					} else {
						this.moveCursorBy(1);
					}
					return true;
				case 263:
					if (Screen.hasControlDown()) {
						...
						int leftSelection = this.cursorPosition < this.selectionEnd ? this.cursorPosition
								: this.selectionEnd;
						this.setCursorPosition(leftSelection);
					} else {
						this.moveCursorBy(-1);
					}
					return true;
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollDelta) {
		this.moveCursorBy((int) Math.signum(scrollDelta));
	...
	public boolean mouseClicked(double p_mouseClicked_1_, double p_mouseClicked_3_, int p_mouseClicked_5_) {
		if (!this.getVisible()) {
			...
			boolean flag = p_mouseClicked_1_ >= (double) this.x && p_mouseClicked_1_ < (double) (this.x + this.width)
					&& p_mouseClicked_3_ >= (double) this.y && p_mouseClicked_3_ < (double) (this.y + this.height);
			int lastCursorPos = this.getCursorPosition();
			this.setCursorPositionZero();
			if (this.canLoseFocus) {
				this.setFocused2(flag);
			}

			if (p_mouseClicked_5_ == 1 && this.isFocused()) {
				this.setText("");
			}
			...
			if (this.isFocused() && flag && p_mouseClicked_5_ == 0) {
				...
				String s = this.fontRenderer.func_238412_a_(this.text.substring(this.lineScrollOffset),
						this.getAdjustedWidth());
				int newCursorPos = this.fontRenderer.func_238412_a_(s, i).length() + this.lineScrollOffset;
				if (alreadyFocused && flag) {
					if (newCursorPos == lastCursorPos || clickCount == 3) {
						switch (clickCount) {
						case 2:
							this.setCursorPosition(this.getNthWordFromCursor(1)
									- ((this.getNthWordFromCursor(1) == this.getText().length()) ? 0 : 1));
				...
				} else if (alreadyFocused || !this.selectOnFocus) {
					this.setCursorPosition(newCursorPos);
				}
				return true;
			} else {
	...
	public void renderButton(MatrixStack stack, int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_) {
		if (this.getVisible()) {
			if (this.getEnableBackgroundDrawing()) {
				fill(stack, this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, -6250336);
			...
			if (!s.isEmpty()) {
				...
				j1 = this.fontRenderer.drawStringWithShadow(stack, this.textFormatter.apply(s1, this.lineScrollOffset),
						(float) l, (float) i1, i);
			}
			...
			if (!s.isEmpty() && flag && j < s.length()) {
				this.fontRenderer.drawStringWithShadow(stack,
						this.textFormatter.apply(s.substring(j), this.cursorPosition), (float) j1, (float) i1, i);
			}
			...
			if (flag1) {
				if (flag2) {
					AbstractGui.fill(stack, k1, i1 - 1, k1 + 1, i1 + 1 + 9, -3092272);
				} else {
	...
	public boolean isMouseOver(double p_isMouseOver_1_, double p_isMouseOver_3_) {
		return this.visible && p_isMouseOver_1_ >= (double) this.x && p_isMouseOver_1_ < (double) (this.x + this.width)
				&& p_isMouseOver_3_ >= (double) this.y && p_isMouseOver_3_ < (double) (this.y + this.height);
	}
	...
	public void setSelectionPos(int position) {
		...
		if (this.fontRenderer != null) {
			...
			if (this.selectionEnd == this.lineScrollOffset) {
				this.lineScrollOffset -= this.fontRenderer.func_238412_a_(this.text.substring(this.lineScrollOffset), j)
						.length();
			}
	...
	public int func_195611_j(int p_195611_1_) {
		return p_195611_1_ > this.text.length() ? this.x
				: this.x + this.fontRenderer.getStringWidth(this.text.substring(0, p_195611_1_));
	}
