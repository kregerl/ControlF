...
public class RichTextFieldWidget extends Widget implements IRenderable, IGuiEventListener {
	...
	public RichTextFieldWidget(FontRenderer fontIn, int xIn, int yIn, int widthIn, int heightIn,
			@Nullable RichTextFieldWidget p_i51138_6_, String msg) {
	...
	public boolean mouseClicked(double p_mouseClicked_1_, double p_mouseClicked_3_, int p_mouseClicked_5_) {
		if (!this.getVisible()) {
			...
			if (this.isFocused() && flag && p_mouseClicked_5_ == 0) {
				...
				int newCursorPos = this.fontRenderer.trimStringToWidth(s, i).length() + this.lineScrollOffset;
	...
	public void renderButton(int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_) {
		if (this.getVisible()) {
			...
			if (!s.isEmpty()) {
				...
				j1 = this.fontRenderer.drawStringWithShadow(this.textFormatter.apply(s1, this.lineScrollOffset),
						(float) l, (float) i1, i);
			}
			...
			if (!s.isEmpty() && flag && j < s.length()) {
				this.fontRenderer.drawStringWithShadow(this.textFormatter.apply(s.substring(j), this.cursorPosition),
						(float) j1, (float) i1, i);
			}
